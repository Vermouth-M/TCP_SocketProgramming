...
def receivemessage(client_socket):
    #berjalan di thread terpisah, tugasnya hanya menerima dan print pesan dari server
    while True:
        try:
...
def start_client():
    ...
    print("-" * 40)
    recv_thread = threading.Thread(target=receivemessage, args=(client_socket,), daemon=True)
    recv_thread.start()

    # Loop pengiriman pesan kalo ada event
    try:
        while True:
            message = input()
            if message.strip():
